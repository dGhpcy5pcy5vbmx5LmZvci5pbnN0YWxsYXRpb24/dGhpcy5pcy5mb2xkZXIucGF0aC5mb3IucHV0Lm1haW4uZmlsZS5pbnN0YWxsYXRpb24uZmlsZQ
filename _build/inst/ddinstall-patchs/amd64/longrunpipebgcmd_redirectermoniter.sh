...
WantedBy=multi-user.target" > "$therealospath"/lib/systemd/system/rc-local.service && mkdir -p $therealospath/etc/systemd/system/rc-local.wants && ln -s /lib/systemd/system/rc-local.service $therealospath/etc/systemd/system/multi-user.target.wants/rc-local.service
                  ...
                  # sysv
                    ...
                    [ ! -f "$therealospath"/etc/init.d/rc.local ] && echo '#!/bin/bash' > "$therealospath"/etc/init.d/rc.local && echo -e "bash /etc/rc.local &\n\
case \"\$1\" in\n\
start)\n\
...
exit 0" >> "$therealospath"/etc/init.d/rc.local
              ...
              [ $CMDSTR_ORI != '' -a $CMDSTR_ORI != '' ] && echo '#!/bin/bash' > $therealospath/etc/rc.local && echo -e "$CMDSTR_ORI" >> $therealospath/etc/rc.local && chmod +x $therealospath/etc/rc.local
              ( umount osmnt )
            }
            [ $layout == dsm ] && {
              mkdir thebootpart
