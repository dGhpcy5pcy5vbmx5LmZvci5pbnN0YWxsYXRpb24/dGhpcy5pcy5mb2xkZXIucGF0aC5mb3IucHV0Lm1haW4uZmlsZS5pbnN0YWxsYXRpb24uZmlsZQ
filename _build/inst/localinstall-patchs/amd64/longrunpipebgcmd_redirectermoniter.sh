...
srchdinfo=`[ \`echo "$srchd"|grep "sd\|vd\|xvd\|nvme"\` ] && echo /dev/"$srchd" || ( for i in \`list-devices disk\`;do [ \`sfdisk --disk-id $i|sed s/0x// |grep -ix $srchd \` ] && echo $i;done|head -n1;exit 0; )`
...
for step in pass1 pass2 pass3; do
    ...
    case $step in
       "pass1")
           ...
           # FIX: Validasi partisi sebelum mount
           if [ ! -b "$srchdinfo$srcptinfo" ]; then
             logger -t pass1 "Partition $srchdinfo$srcptinfo not found!"
             exit 1
           fi
           ...
           # FIX: Set default jika kosong dan validasi angka
           filesize=${filesize:-0}
           disksize=${disksize:-0}
           ptsize=${ptsize:-0}
           diskavaliable=${diskavaliable:-0}
           partavaliable=${partavaliable:-0}

           if ! echo "$filesize$disksize$ptsize$diskavaliable$partavaliable" | grep -Eq '^[0-9]+$'; then
             logger -t pass1 "Invalid numeric values, aborting."
             exit 1
           fi
           ...
           if [ "$diskavaliable" -ge "$filesize" ]; then
             ...
             # FIX: Validasi angka dan kondisi
             needborrowsize=${needborrowsize:-0}
             borrowoffset=${borrowoffset:-0}

             if ! echo "$partavaliable$needborrowsize$borrowoffset" | grep -Eq '^[0-9]+$'; then
               logger -t pass1 "Invalid numeric computation values."
               exit 1
             fi
